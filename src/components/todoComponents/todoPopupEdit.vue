                    ...
                    <v-menu max-width="290">
                        <template v-slot:activator="{ on }">
                            <v-text-field :value='formatedDate' v-on="on" label='Due date' prepend-icon='mdi-calendar'
                                :rules='inputRules' :placeholder="duePlaceholder"></v-text-field>
                        </template>
                        <v-date-picker v-model="due"></v-date-picker>
                    </v-menu>
                    <v-row>
                        <v-col cols="12" sm="6">
                            <v-select chips :items="items" label="Status" prepend-icon='mdi-check' v-model="status"
                                :rules='inputRules'></v-select>
                        </v-col>
                        <v-col cols="12" sm="6">
                            <v-select chips :items="people" label="Person" prepend-icon='mdi-account' v-model="person"
                                :rules='inputRules'></v-select>
                        </v-col>
                    </v-row>
                    <v-spacer></v-spacer>
